...
from src.streamlit.shared.team_helpers import add_team_flag_column
...
# ─────────────────────────────────────────────────────────────
# Top N summary functions (non-pass events remain inline here)
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# Pass metrics via CompetitionPassMetrics
# ─────────────────────────────────────────────────────────────
pass_metrics = CompetitionPassMetrics(df)
top_passers_df = pass_metrics.top_passers()
top_assisters_df = pass_metrics.top_assisters()
pass_success_rate = pass_metrics.pass_success_rate()
...
col2.metric("Top Assister", top_assisters_df.iloc[0]["player"], f'{top_assisters_df.iloc[0]["assists"]} assists')
col3.metric("Most Passes", top_passers_df.iloc[0]["player"], f'{top_passers_df.iloc[0]["passes"]} passes')
...
col2.dataframe(top_assisters_df, use_container_width=True)

col3.subheader("Most Passes")
col3.dataframe(top_passers_df, use_container_width=True)
